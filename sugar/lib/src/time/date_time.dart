...
class DateTimeBase implements Date, Time {
  ...
  DateTimeBase.fromNative(DateTime date)
      : _native = DateTime.utc(date.year, date.month, date.day, date.hour,
            date.minute, date.second, date.millisecond, date.microsecond);

  /// Creates a [DateTimeBase].
  DateTimeBase(int year,
      [int month = 1,
      int day = 1,
      int hour = 0,
      int minute = 0,
  ...
  DateTimeBase._(this._native)
      : assert(_native.isUtc,
            '$_native is in local timezone. Should be UTC. $issueTracker');
