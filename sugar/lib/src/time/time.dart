...
abstract class Time {
  ...
  Time.fromDayMilliseconds(DayMilliseconds milliseconds)
    : _native = DateTime.fromMillisecondsSinceEpoch(milliseconds, isUtc: true);

  /// Creates a [Time] with the microseconds since midnight.
  Time.fromDayMicroseconds(DayMicroseconds microseconds)
    : _native = DateTime.fromMicrosecondsSinceEpoch(microseconds, isUtc: true);

  /// Creates a [Time] from the native [DateTime].
  Time.fromNative(DateTime time)
    : _native = DateTime.utc(1970, 1, 1, time.hour, time.minute, time.second, time.millisecond, time.microsecond);
